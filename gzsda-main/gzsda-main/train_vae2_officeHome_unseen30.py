...
class TwoModalDataset(Dataset):
    ...
    def load_mat(self,sourceDomainIndex=0, targetDomainIndex=0,trialIndex=0):
        ...
        dataSplit = scipy.io.loadmat(data_dir+'instanceSplit_officehome_unseen30.mat')# default data split for office-home, 30 unseen classes
...
def main(args):
    ...
    vae = VAE2(
        ...
        num_domains = 2,
        num_labels=65 if args.conditional else 0).to(device)    
    ...
    def next_batch(vae,batch_size=64):
        y = np.random.randint(65,size=batch_size)
    ...
    num_epochs = 50
    acc_per_class = np.zeros((num_epochs,65))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sourceDomainIndex", type=int, default=3)
    ...
    args.filename = 'officeHome30-'+source+'-'+target+'-trial'+str(args.trialIndex)+'-vaeEpochs-'+str(args.epochs)+'-latSize-'+str(args.latent_size)+'lr'+str(args.learning_rate)
    print(args.filename)
    main(args)
